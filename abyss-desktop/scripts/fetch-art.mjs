#!/usr/bin/env node
/* Downloads the 22 public-domain paintings into assets/art/.
   Run once per machine; `npm run fetch:art`. */
...
async function download(url, dest, timeoutMs = 30000) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.get(url, {
      headers: {
        'User-Agent': 'AbyssDesktopFetchArt/0.1 (edenadiv1@gmail.com)',
        'Accept': '*/*',
      },
    }, (res) => {
...
async function run() {
  ensureDir(OUT);
  const failures = [];
  console.log(`[fetch-art] downloading ${Object.keys(SOURCES).length} paintings to ${OUT}`);
  ...
  if (failures.length) {
    console.log(`\n[fetch-art] ${failures.length} failed. Re-run when the network is available.`);
    process.exit(1);
  }
  console.log('\n[fetch-art] all paintings bundled.');
}
