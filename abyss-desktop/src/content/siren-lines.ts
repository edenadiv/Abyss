/* Siren dialogue — three styles: seductive, cryptic, mocking. Player
   settles one in Settings → Game. The key is the situation trigger. */

export type SirenStyle = 'seductive' | 'cryptic' | 'mocking';
export type SirenTrigger =
  | 'greet' | 'bet_place' | 'win' | 'loss' | 'low_breath' | 'high_breath';

type LineBook = Record<SirenTrigger, string[]>;
export const SIREN_LINES: Record<SirenStyle, LineBook> = {
  seductive: {
    greet: [
  ...
  cryptic: {
    ...
    win: ['Borrowed.', 'The sea is not pleased.', 'Keep it. For now.'],
    loss: ['As expected.', 'It takes. It always takes.', 'This is how the city was built.'],
    low_breath: [
  ...
  mocking: {
    ...
    win: ['Huh. The house blinked.', 'Fluke. Entirely fluke.', "Don't look so smug. The water's still rising."],
    loss: ['There it is. I almost missed it.', 'Predictable. Honestly.', 'Oh no. Anyway.'],
    low_breath: [
...
export function pickLine(trigger: SirenTrigger, style: SirenStyle = 'seductive'): string {
  const pool = SIREN_LINES[style]?.[trigger] ?? SIREN_LINES.seductive[trigger] ?? [];
